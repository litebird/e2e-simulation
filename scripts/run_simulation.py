...
nsims           = sys.argv[2].zfill(4) #how many simulations to do
isimend         = str(int(isimstart)+int(nsims)-1).zfill(4)
...
for isim in np.arange(int(isimstart),int(isimend)+1):
    #get base_path
    base_path = base_path_prefix+'sim'+str(isim).zfill(4)+'/'+det_names_file+'/'

    toml_filename = 'e2e_sim'+str(isim).zfill(4)+'_'+det_names_file+'_params'
...
if(mapmaking_type=='all' or mapmaking_type=='destriper'):
    ...
    for isim in np.arange(int(isimstart),int(isimend)+1):
        ...
        #loop over maps to do
        for madam_map in madam_maps_list:
            slurm_madam = coderoot+'slurm_madam_'+madam_map+'_sim'+str(isim).zfill(4)+'_'+det_names_file+'.sl'
