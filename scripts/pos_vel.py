...
def pos_vel(toml_filename):
    ...
    sim = lbs.Simulation(
        parameter_file=os.path.dirname(os.getcwd())
        + "/ancillary/"
        + toml_filename
        + ".toml",
    )
    ...
    start_time = Time(
        sim.parameters["simulation"]["start_time"]
    )  # simulation start time
    mission_time_days = sim.parameters["general"][
        "mission_time_days"
    ...
    np.savetxt(
        base_path + "spacecraft_pos_vel_" + mission_time_days + "d.txt",
